...
// This function is called on document load through turbolinks load
// Called in last line of this javascript file.
var ready = function() {
...
*/
  ...
  function init() {
    ...
    // As these history and duplicate elements are added after the page load,
    ...
    $(".duplicates").on('click', '#closeduplicate' , function() {
      console.log("closing duplicate clicked");
      $(this).parent("div").remove();
    });
    ...
    var duplicateElement = document.getElementById("d1");
    if(duplicateElement){
      duplicateElement.style.position="fixed";
      duplicateElement.style.left = "2%";
      duplicateElement.style.top  = "10%";
    }
  }
...
*/
    ...
    switch( thisAction ) {     
      ...
      case "removerun":
        if(!confirm("Are your sure?  \n" + 
            "Are so sure you wish to delete THIS ENTITY and then everything for the rest of this chain \n" +
  ...
  // Draggable history container
    ...
    $(".duplicates").on('mouseover', '.duplicate', function(){
      $(this).draggable();
    });
  ...
  function personupdateslesson_Update( domchange ){
    ...
    $.ajax({
        ...
        success: function(result1, result2, result3){
            ...
            //var domchange  = result1['domchange'];
            if(('duplicates' in result1) &&
              (result1['duplicates'] != null) &&
               (result1['duplicates'].length > 0)){
              var duplicates = result1['duplicates'];
              console.log("duplicates present.");
              showduplicate(duplicates);
            }
            //moveelement_update(domchange);
        },
        error: function(xhr){
  ...
  function personupdateslesson_Update_stats( domchange ){
    ...
    $.ajax({
        ...
        success: function(result1, result2, result3){
            ...
            var domchange  = result1['domchange'];
            stats_student_update(domchange);
            if(('duplicates' in result1) && (result1['duplicates'].length > 0)){
              var duplicates = result1['duplicates'];
              console.log("duplicates present.");
              showduplicate(duplicates);
            }
        ...
        error: function(xhr){
            ...
            //}
            //alert("error moving student or tutor to another lesson: " + error_message);
            alert("error moving student or tutor to another lesson: " + xhr.responseText);
        }
     });
  }
...
function positionMenu(thismenu) {
  ...
  function showduplicate(duplicatedata){
    var template_ele = document.getElementById("duplicate-template");
    var this_ele = template_ele.cloneNode(true);
    this_ele.id = 'd' + (+new Date);    // current unix time in msec - provides uniqueness.
    this_ele.innerHTML = duplicateToHtml(duplicatedata);
    template_ele.after(this_ele);
    var duplicateDisplay = this_ele;
    //positionMenu(duplicateDisplay);
    duplicateDisplay.style.position="fixed";
    duplicateDisplay.style.left = "2%";
    duplicateDisplay.style.top  = "10%";
  ...
  function duplicateToHtml(dd){
    ...
    htmlsegment += '<div id="closeduplicate"><svg height="300" width="300"><line x1="1" y1="1" x2="15" y2="15" style="stroke:#000; stroke-width:4" /><line x1="15" y1="1" x2="1" y2="15" style="stroke:#000; stroke-width:4" /></svg></div>';
    htmlsegment += '<table>';
    htmlsegment += '<tr><th>Slot</th><th>Name</th><th>Type</th></tr>';
    dd.forEach( function(dup){
      htmlsegment += "<tr><td>" + dup[2] + "</td>";
      htmlsegment += "<td>" + dup[1] + "</td>";
      htmlsegment += "<td>" + dup[0] + "</td></tr>";
    });
    htmlsegment += '</table>';
    return htmlsegment;
  }
