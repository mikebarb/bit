...
class SlotsControllerTest < ActionController::TestCase
  ...
  test "should create slot" do
     # need to delete before update as records must be unique due to db contraints 
     delete :destroy, id: @slot
     assert_difference('Slot.count') do
      post :create, slot: { comment: @slot.comment, location: @slot.location, timeslot: @slot.timeslot }
    end
  ...
  test "should update slot" do
    patch :update, id: @slot, slot: { comment: @slot.comment, location: @slot.location, timeslot: @slot.timeslot }
    assert_redirected_to slot_path(assigns(:slot))
  end
